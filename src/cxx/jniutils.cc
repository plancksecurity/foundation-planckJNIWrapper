...
jobject from_cache_entry(JNIEnv *env, PassphraseCache::cache_entry entry)
{
    ...
    jobject obj = env->NewObject(clazz, constructor);
    env->DeleteLocalRef(clazz);
    _setStringField(env, classname, obj, "email", entry.email.c_str());
    _setStringField(env, classname, obj, "passphrase", entry.passphrase.c_str());
    return obj;
}
...
{
    if (!obj) {
        return PassphraseCache::cache_entry("", ""); // Return an empty entry
    }
    ...
    return PassphraseCache::cache_entry(email, passphrase);
}
